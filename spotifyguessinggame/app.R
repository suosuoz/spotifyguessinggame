...
ui <- fluidPage(
  ...
  sidebarPanel(
    p("The goal of this game is to simply guess the artist or title of a song. If there are multiple artists listed on the song, you only need enter one of them. You will have thirty seconds to guess each song. If time runs out, please hit the submit button to go to the next track or submit an answer. Once you finish a playlist, you can enter a new playlist URI or reenter the same playlist URI to play it again. Not all tracks have listening previews and are therefore cannot be included. Finally, there is a little bit of leniency, so don't worry if you mispell an artist or track name!"),
    br(),
    p("Instructions: "),
    tags$ol(
  ...
  mainPanel(
    ...
    fluidRow(column(8, align="center", offset=2, 
                    actionButton("artistbutton", "Guess the Artist"),
    ...
    fluidRow(column(8, align="center", offset=2, 
                    textInput("txt21", label="Enter a Playlist Link",))),
    fluidRow(column(8, align="center", offset=2, 
                    actionButton("selectbutton1", "Submit"))),
    br(),
    ...
    br(),
    fluidRow(column(8, align="center", offset=2,
                    actionButton("switchtoartist", "Switch to Artist"))),
    fluidRow(column(8, align="center", offset=2,
                    actionButton("switchtosong", "Switch to Song"))),
    
    br(),
    fluidRow(column(8, align="center", offset=2,
                    textInput("txt1", "Guess the Song Title"))),
    fluidRow(column(8, align="center", offset=2,
                    actionButton("guessbutton", "Submit"))),
    fluidRow(column(8, align="center", offset=2,
                    textInput("txt11", "Guess the Artist"))),
    fluidRow(column(8, align="center", offset=2,
                    actionButton("guessbutton1", "Submit"))),
    br(),
    br(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$guessbutton, {
    ...
    #   n
    if (txtcheckxx == TRUE
        #| txtcheckx2 == TRUE | txtcheckx3 == TRUE | txtcheckx4 == TRUE | txtcheckx5 == TRUE
        ) {
      score <<- score + 1
      y <- 0
      ret <- "Correct!"
  ...
  observeEvent(input$guessbutton1, {
    ...
    #Create a new row on the table
    newEntry <- observe({
      isolate(n$df[nrow(n$df) + 1, ] <- c(song, artist[1, 1], input$txt11, ret2))
    })
